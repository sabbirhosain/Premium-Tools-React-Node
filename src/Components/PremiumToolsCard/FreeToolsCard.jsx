...
import DEFAULT_IMAGE from '../../assets/tools_details.png'
import { Link } from "react-router-dom";
import './PremiumToolsCard.css';

const FreeToolsCard = ({ item }) => {
    return (
        <div className="col-lg-3 col-md-4 col-sm-6 mb-4">
            <div className='tools_service_card_box'>
                ...
                <div className='tools_service_text_box'>
                    <Link to={`/free-tools/tools-details/${item._id}`} className='tools_service_title'>{item.item_name}</Link>
                    ...
                    <div className='d-flex align-items-center gap-1 pt-3'>
                        <Link to={`/free-tools/check-out/${item._id}`} className='btn btn-success w-100 rounded-0 btn-sm'>BUY NOW</Link>
                        <Link to={`/free-tools/tools-details/${item._id}`} className='btn btn-outline-success w-100 rounded-0 btn-sm'>DETAILS</Link>
                    </div>
                </div>
            </div>
        </div>
